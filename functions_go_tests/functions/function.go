...
func GetBoardgameNames(w http.ResponseWriter, r *http.Request) {
	...
	// Ensure the response is a valid JSON array, even if empty
	w.Header().Set("Content-Type", "application/json")
	if len(boardgames) == 0 {
		log.Println("No boardgames found, returning empty list")
	...
	// Send the JSON response containing the boardgame data
